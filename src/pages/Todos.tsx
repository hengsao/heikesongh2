    ...
    <div className="page-shell space-y-6">
      <section className="glass-card p-6">
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-[220px_1fr]">
          ...
          <div className="rounded-2xl bg-white/75 p-4 text-sm font-bold text-zinc-600">
            今日待完成 {active.length} 条，已完成 {completed.length} 条
          </div>
        </div>
      </section>
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div className="min-w-0">
                <div className="mb-2 flex flex-wrap gap-2">
                  {item.category ? <span className="rounded-full bg-orange-50 px-3 py-1 text-xs font-bold text-orange-700">{item.category}</span> : null}
                  {item.isPinned ? <span className="rounded-full bg-ink px-3 py-1 text-xs font-bold text-white">置顶</span> : null}
                </div>
                <h3 className={`text-xl font-black ${completed ? "text-zinc-400 line-through" : "text-ink"}`}>{item.title}</h3>
                <p className="mt-2 text-sm leading-7 text-zinc-600">{item.description || "完成后可以转成一张人生卡。"}</p>
              </div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                <button className="icon-button" title="编辑" onClick={() => onEdit(item.id)}><Edit3 size={17} /></button>
                <button className="icon-button" title="删除" onClick={() => onRemove(item.id)}><Trash2 size={17} /></button>
              </div>
            </div>
          </article>
        )) : (
          <div className="glass-card p-7 text-sm text-zinc-500">这里暂时没有事项。</div>
        )}
      </div>
    </section>
  );
